<div align="center">

# Ahmed Javed
### Junior Cybersecurity Professional

*Information Security · Defensive Security · Offensive Security · Penetration Testing*

[![TryHackMe](https://img.shields.io/badge/TryHackMe-Active-red?style=for-the-badge&logo=tryhackme&logoColor=white)](https://tryhackme.com)
[![HackTheBox](https://img.shields.io/badge/HackTheBox-Active-9FEF00?style=for-the-badge&logo=hackthebox&logoColor=black)](https://hackthebox.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com)
[![ISC2](https://img.shields.io/badge/ISC2-CC%20Certified-006400?style=for-the-badge&logoColor=white)](https://isc2.org)

</div>

---

## 👋 About Me

I am a **Cybersecurity Professional in Training** focused on information security, defensive security, offensive security, and penetration testing, with a strong interest in SOC operations, threat awareness, and real-world security implementation.

My background combines **technical precision, attention to detail, and clear communication** — developed through experience in post-production and client-facing roles. These skills now support my transition into cybersecurity, where accuracy, analysis, and responsibility are essential.

I am actively building foundational and hands-on knowledge in **network defense, threat analysis, vulnerability assessment, and security monitoring**, while learning how attackers think in order to build stronger defenses.

> 🎯 **Goal:** Grow into an entry-level Cybersecurity or SOC Analyst role, contribute to protecting digital assets, and continue developing skills in penetration testing and offensive security.

---

## 🔭 Current Focus Areas

```
├── Cybersecurity Fundamentals & Information Security
├── SOC Operations & Security Monitoring
├── Defensive Security & Network Defense
├── Offensive Security & Penetration Testing Fundamentals
├── Threat Awareness & Threat Intelligence Basics
├── Vulnerability Assessment & Security Hardening
├── Log Analysis, Anomaly Detection & Incident Awareness
├── Risk Management, Security Principles & Governance
└── Communicating Technical Security Risks to Business Stakeholders
```

---

## 🏅 Certifications

| Certificate | Issuer | Status |
|---|---|---|
| CC — Certified in Cybersecurity | ISC2 | ✅ Completed |
| Cybersecurity & Ethical Hacking Diploma | Boston Institute of Analytics | 🔄 In Progress (Dec 2025 – Apr 2026) |
| Cybersecurity Awareness & Terminology | LinkedIn Learning | ✅ Completed |
| ISC2 Candidate Badge | ISC2 | ✅ Active |

**CC — Domains Covered**

`Domain 1: Security Principles` · `Domain 2: Incident Response, BC & DR` · `Domain 3: Access Control Concepts` · `Domain 4: Network Security`

---

## ⚔️ Hands-On Practice

I actively sharpen practical skills through CTF challenges and labs:

[![TryHackMe](https://img.shields.io/badge/TryHackMe-Rooms%20Completed-red?style=flat-square&logo=tryhackme&logoColor=white)](https://tryhackme.com)
[![HackTheBox](https://img.shields.io/badge/HackTheBox-Active-9FEF00?style=flat-square&logo=hackthebox&logoColor=black)](https://hackthebox.com)

**My methodology:**
```
Recon  →  Enumeration  →  Exploitation  →  Post-Exploitation  →  Document
```

📁 See my [CTF Writeups](./ctf-writeups) — documented walkthroughs showing my thinking, not just the flags.

---

## 🛠️ Tools & Technologies

**Offensive & Recon**

`Nmap` · `Burp Suite` · `Metasploit` · `Gobuster` · `Nikto` · `Netcat` · `OSINT Techniques` a. `Nessus` . `John the Ripper` . `Nmap`

**Defensive & Monitoring**

`Wireshark` · `Log Analysis` · `Anomaly Detection` · `Incident Response Basics`

**Networking & OS**

`TCP/IP` · `DNS` · `HTTP/S` · `Linux` · `Windows` · `Network Protocols`

**Concepts**

`OWASP Top 10` · `Privilege Escalation` · `Vulnerability Assessment` · `Risk Management` · `Security Governance` · `Threat Intelligence` . `Mitre Att&ck`

---

## 📂 Featured Projects

| Project | Description |
|---|---|
| 📝 [CTF Writeups](https://github.com/YOUR_USERNAME/ctf-writeups) | TryHackMe & HackTheBox walkthroughs with full methodology |
| 🔍 [Project 2](https://github.com/YOUR_USERNAME/project2) | *(Add your project here)* |
| 🛡️ [Project 3](https://github.com/YOUR_USERNAME/project3) | *(Add your project here)* |

---

## 📊 GitHub Stats

<div align="center">

![Ahmed's GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=dark&hide_border=true&count_private=true&bg_color=0d1117&title_color=00ff41&icon_color=00ff41)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=dark&hide_border=true&background=0d1117&ring=00ff41&fire=00ff41&currStreakLabel=00ff41)

</div>

---

## 🤝 Let's Connect

I enjoy learning, collaborating, and connecting with others who value **strong fundamentals, practical security, and continuous improvement.**

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Ahmed%20Javed-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com)
[![TryHackMe](https://img.shields.io/badge/TryHackMe-Profile-red?style=for-the-badge&logo=tryhackme&logoColor=white)](https://tryhackme.com)
[![HackTheBox](https://img.shields.io/badge/HackTheBox-Profile-9FEF00?style=for-the-badge&logo=hackthebox&logoColor=black)](https://hackthebox.com)

</div>

---

<div align="center">

*"The quieter you become, the more you are able to hear." — Kali Linux*

![Visitor Count](https://komarev.com/ghpvc/?username=YOUR_USERNAME&color=00ff41&style=flat-square&label=Profile+Views)

</div>
